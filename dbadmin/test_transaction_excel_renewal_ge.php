...
$sql="SELECT im.*, ad.branch_code FROM installment_master_ge_renewal as im JOIN admin as ad ON ad.id = im.branch_id WHERE is_deleted=0 ".$where." ORDER BY id DESC";
 //echo $sql;
// exit;
...
$numTransaction = mysql_num_rows($selTransaction);
	...
	//$rs = $objDB->select();
		...
		*/
		...
		$header .= "Cheque/DD Branch Name."."\t";
         ...
         $header .= "Cash Money Receipt."."\t";
		$header .= "Cheque Money Receipt."."\t";		
		$header .= "Draft Money Receipt."."\t";
            $header .= "Health"."\t";
		...
		if($numTransaction > 0)
		{
			while($getTransaction = mysql_fetch_assoc($selTransaction))
			{
				...
				$Query = "select campaign,campaign_code from t_99_campaign WHERE campaign_id=$getTransaction[campaign_id]";
				//echo $Query;
				$objDB->setQuery($Query);
				...
				$line .= $getTransaction['policy_no']."\t";
				$due_date = '';
				if(($getTransaction['due_date'] != '0000-00-00') && ($getTransaction['due_date'] != '1970-01-01')){
				...
				$line .= $getTransaction['health']."\t";
				*/
				
				$line .= "'".stripslashes($getTransaction['policy_no'])."\t";
				...
				else
				{
					$line .= "Yes"."\t";
								...
								$due_date = '';
				if($getTransaction['due_date'] != '0000-00-00'){
					$due_date = date('d/m/Y',strtotime($getTransaction['due_date']));
				}
				$line .= $due_date."\t";
				...
				if($getTransaction['receive_cheque']>0)
				{
					...
					$line .= $cheque_date."\t";
					$line .= stripslashes($getTransaction['cheque_bank_name'])."\t";
					$line .= stripslashes($getTransaction['cheque_branch_name'])."\t";
				}
				else
				{
					$line .= "'".$getTransaction['dd_no']."\t";
	...
	header("Content-Disposition: attachment; filename=renewal_ge".'_'.date('m-d-Y_H:i').".xls");
