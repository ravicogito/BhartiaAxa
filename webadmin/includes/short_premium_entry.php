...
if($msg=="")
{
			...
			foreach($xml->GTFScolldata as $v)
			{
				...
				$type_of_business=addslashes($v->transaction_type);
				//$application_no=$v->application_no;
				
				
				
				$application_no=addslashes($v->application_no);
				$policy_no=addslashes($v->policy_no);
				$company_code=addslashes($v->agent_code);
				
				if($company_code!='2000002609')
				{
					$_SESSION['error_msg']="This company name should be SZ. Please review the .xml file.";
					header("location: ".URL.'webadmin/index.php?p=short_premium_entry_sz');
					exit;
				}
				
				$money_receipt=addslashes($v->perm_receipt_no);
				
				$applicant_name=addslashes($v->policy_holder);
				//$applicant_dob=$v->DOB;
				...
				$plan_name=addslashes($v->Product_Name);
				
				$receive_amount=addslashes($v->amount);
				$receive_mode=addslashes($v->pay_mode); // csh, chq, dft
				$cheque_no=addslashes($v->cheque_no);
				$cheque_date=addslashes($v->cheque_date);
				if($cheque_date!="")
				{
					$cheque_date=date("Y-m-d", strtotime($cheque_date));
				}
				$cheque_bank=addslashes($v->bank_name);
				$cheque_branch=addslashes($v->bank_branch);
				
				$reciept_status=addslashes($v->Reciept_Status);
				...
				//$tot_receipt=mysql_num_rows(mysql_query("select id from  installment_master_short_premium where $database_field='$money_receipt' and is_deleted='0'"));
				
				$tot_receipt=mysql_num_rows(mysql_query("select id from installment_master_short_premium where (cash_money_receipt='$money_receipt' || cheque_money_receipt='$money_receipt' || draft_money_receipt='$money_receipt') and is_deleted='0'"));
				
				//-------- end for check receipt no -----------//
				
				// if($tot_receipt==0 && (strtolower($type_of_business)=='rwp'|| strtolower($type_of_business)=='fpr'))
				if($tot_receipt==0 && strtolower($type_of_business)!='rwp' && strtolower($type_of_business)!='frp' && $policy_no=="" && strtolower($reciept_status)=='live')
				{
				
					//-------- start for check application number ---------//
						$query_policy_no=mysql_query("select id from installment_master_short_premium where application_no='$application_no' and is_deleted='0'");
					...
					// if($tot_policy_no==0) 
					{
						...
						*/
						...
						mysql_query("insert into installment_master_short_premium set $query");
					...
					/*else
					{
						...
						mysql_query("update installment_master_short_premium set $query where id='$id'");
			...
			header("location: ".URL.'webadmin/index.php?p=short_premium_list');
}
...
<TABLE class="table_border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <tbody>
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Manual Entry (Short Premium)</td>
    </tr>
