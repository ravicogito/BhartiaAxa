...
if($msg=="")
{
			...
			foreach($xml->GTFScolldata as $v)
			{
				...
				if($company_code!='2000002609')
				{
					...
					header("location: ".URL.'webadmin/index.php?p=renewal_entry_sz');
					exit;
				}
				*/
				...
				$due_date=addslashes($v->due_date);
				if($due_date!="")
				{
				...
				//$tot_receipt=mysql_num_rows(mysql_query("select id from  installment_master_sz_renewal where $database_field='$money_receipt' and is_deleted='0'"));
				
				$tot_receipt=mysql_num_rows(mysql_query("select id from installment_master_sz_renewal where (cash_money_receipt='$money_receipt' || cheque_money_receipt='$money_receipt' || draft_money_receipt='$money_receipt') and is_deleted='0'"));
				
				//-------- end for check receipt no -----------//
				
				if($tot_receipt==0 && (strtolower($type_of_business)=='rwp'|| strtolower($type_of_business)=='arn')  && strtolower($reciept_status)=='live' && $company_code=='2000002609')
				{
				
					//-------- start for check application number ---------//
						$query_policy_no=mysql_query("select id from installment_master_sz_renewal where policy_no='$policy_no' and is_deleted='0'");
					...
					// if($tot_policy_no==0) 
					{
						...
						*/
						...
						mysql_query("insert into installment_master_sz_renewal set $query");
					...
					/*else
					{
						...
						mysql_query("update installment_master_sz_renewal set $query where id='$id'");
			...
			header("location: ".URL.'webadmin/index.php?p=renewal_list_sz');
}
...
<TABLE class="table_border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <tbody>
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Manual Entry (Renewal - SZ)</td>
    </tr>
