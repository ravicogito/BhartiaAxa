...
function show_list(){
	...
	$sortField 		= loadVariable('sf','business_type');
...
$where = " WHERE 1 ";
	if($searchField=="business_type"){
		$where .= " AND business_type = '".mysql_real_escape_string($searchString)."' ";
	}
//echo $where;
$OrderBY = " ";
if($sortField <> "" && $sortType <> "")
{
	$OrderBY .= " ORDER BY business_type ".$sortType;
}
//=======================================================
$Query = "select count(id) as CNT from business_type  ".$where;
...
$Query = "select * from business_type ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Insurance Plan Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="business_type" <?php if($searchField=='business_type') echo 'selected';?>>Business Type</option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=business_type&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><? include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=business_type&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> Plan </TD>
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['business_type']?></TD>
	  <!-- <TD align=center>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM business_type WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Business Type <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="business_type" id="business_type" size="40" <?php if(isset($rec[0]['business_type']) && $rec[0]['business_type']!="") {?> value="<?=stripslashes($rec[0]['business_type'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
			<tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Is Latest Business Type </td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<!--<input type="text" class="validate[required] inplogin" name="plan_name" id="plan_name" size="40" <? if(isset($rec[0]['plan_name']) && $rec[0]['plan_name']!="") {?> value="<?=stripslashes($rec[0]['plan_name'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />--><input type="checkbox" name="is_new" id="is_new" value="1" <?php if(isset($rec[0]['is_new'])){ echo ($rec[0]['is_new'] == '1' ? 'checked ' : '');}?>>
				</td>
			  </tr>
			</table>
		</td>
...
{
	$objDB = new DB();
	$business_type 					= loadVariable('business_type','');
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from business_type WHERE business_type = '".$business_type."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "Plan already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
		...
		//================================
		$_SESSION[SUCCESS_MSG] = "Plan Added successfully...";
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
	else
	{
		...
		$Query  = "select * from business_type WHERE business_type = '".$business_type."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$_SESSION[SUCCESS_MSG] = "Plan updated successfully...";
		$objDB->close();
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from business_type WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	if ($rsu[0]['status']== 1)
	{
	$Query  = "UPDATE business_type SET status = 0 WHERE id = ".$row_id."";
	}
	elseif ($rsu[0]['status']== 0)
	{
	$Query  = "UPDATE business_type SET status = 1 WHERE id = ".$row_id."";
	}
	#echo $Query;
	#die();
	$objDB->setQuery($Query);
	$rs = $objDB->update();
	$_SESSION[SUCCESS_MSG] = "Plan Status Changed successfully...";
	header("location: index.php?p=".$_REQUEST['p']."");
	exit();
	}
}
