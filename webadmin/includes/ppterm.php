...
function show_list(){
	...
	$sortField 		= loadVariable('sf','paying_term');
...
$where = " WHERE 1 ";
	...
	if($searchField=="paying_term"){
		$where .= " AND paying_term = '".mysql_real_escape_string($searchString)."' ";
	}
...
//=======================================================
$Query = "select count(id) as CNT from premium_paying_term_master  ".$where;
...
$Query = "select * from premium_paying_term_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
  ...
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="paying_term" <? if($searchField=='paying_term') echo 'selected';?>>Premium Paying Term </option></td>
      <TD width="85%">
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> Premium Paying Term </TD>
		<!-- <TD  width="15%" class="heading_text" align="left"> Bonus Percentage </TD>
...
<!-- <TD align=center width="12%" class="heading_text">Delete</TD> -->
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['paying_term']?></TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM premium_paying_term_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Premium Paying Term <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="paying_term" id="paying_term" size="40" <? if(isset($rec[0]['paying_term']) && $rec[0]['paying_term']!="") {?> value="<?=stripslashes($rec[0]['paying_term'])?>" <? }?> onKeyPress="return keyRestrict(event, '0123456789')" />
				</td>
			  </tr>

				<!-- <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Bonus Percentage <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="bonus_percentage" id="bonus_percentage" size="40" <? if(isset($rec[0]['bonus_percentage']) && $rec[0]['bonus_percentage']!="") {?> value="<?=stripslashes($rec[0]['bonus_percentage'])?>" <? }?> onKeyPress="return keyRestrict(event, '0123456789.')" />
				</td> -->
			  </tr>
...
function  update_record($row_id = '')
{
	...
	$paying_term 					= loadVariable('paying_term','');
	$bonus_percentage 					= loadVariable('bonus_percentage','');
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from premium_paying_term_master WHERE paying_term = '".$paying_term."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "Premium Paying Term already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
				
					
		$Query  = " INSERT INTO premium_paying_term_master SET ";
		$Query .= " paying_term 							= '".$paying_term."',";
		$Query .= " bonus_percentage 							= '".$bonus_percentage."',";
		$Query .= " status 							= 1,";
		$Query .= " activation_date 							= '".date('Y-m-d')."',";
		$Query .= " deactivation_date 							= '".date('0000-00-00')."'";
		...
		//================================
		$_SESSION[SUCCESS_MSG] = "Premium Paying Term Added successfully...";
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
	else
	{
		...
		$rsu = $objDB->select();*/
				...
				//echo $cat_name;
				//exit;
		if($paying_term == '' && $bonus_percentage == '')
		{	
			$_SESSION[ERROR_MSG] = "Please enter data for all mandetory fields...";
			//cat_add_edit($row_id);
		...
		$Query  = "select * from premium_paying_term_master WHERE paying_term = '".$paying_term."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$Query  = " UPDATE premium_paying_term_master SET ";
		$Query .= " paying_term							= '".$paying_term."',";
		$Query .= " bonus_percentage							= '".$bonus_percentage."'";
		...
		$_SESSION[SUCCESS_MSG] = "Premium Paying Term updated successfully...";
		$objDB->close();
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from premium_paying_term_master WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	...
	$Query  = "UPDATE premium_paying_term_master SET status = 0, deactivation_date='".date('Y-m-d')."' WHERE id = ".$row_id."";
	#mysql_query("UPDATE premium_paying_term_master SET deactivation_date='".date('Y-m-d')."'");
	...
	$Query  = "UPDATE premium_paying_term_master SET status = 1, deactivation_date='0000-00-00' WHERE id = ".$row_id."";
	}
	...
	$_SESSION[SUCCESS_MSG] = "Premium Paying Term Status Changed successfully...";
