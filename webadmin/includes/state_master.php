...
function show_list(){
	...
	$sortField 		= loadVariable('sf','state_name');
...
$where = " WHERE 1 ";
	if($searchField=="state_name"){
		$where .= " AND state_name = '".mysql_real_escape_string($searchString)."' ";
	}
//echo $where;
$OrderBY = " ";
...
//=======================================================
$Query = "select count(id) as CNT from state_master  ".$where;
...
$Query = "select  *  from state_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>State Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="state_name" <? if($searchField=='state_name') echo 'selected';?>>State </option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=state_master&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><? include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=state_master&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> State </TD>
<!--          <TD  width="15%" class="heading_text" align="left"> State Code </TD>-->
...
<!--      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['state_code']?></TD>-->
	  <!-- <TD align=center>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM state_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
			...
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				...
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="state_name" id="state_name" size="40" <? if(isset($rec[0]['state_name']) && $rec[0]['state_name']!="") {?> value="<?=stripslashes($rec[0]['state_name'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
                          
<!--                          <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">State Code</td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="state_code" id="state_code" size="40" <? if(isset($rec[0]['state_code']) && $rec[0]['state_code']!="") {?> value="<?=stripslashes($rec[0]['state_code'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>-->
                          
<!--			<tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Is Latest State </td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
...
{
	$objDB = new DB();
	$state_name 					= loadVariable('state_name',''); //state_code
//        $state_code 					= loadVariable('state_code','');
//	$is_new 					= loadVariable('is_new','');
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from state_master WHERE state_name = '".$state_name."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
...
//			$is_new = 0;
//			}
		$Query  = " INSERT INTO state_master SET "; //state_code
...
//		$Query .= " is_new                                                      = '".$is_new."'";
	...
	else
	{
		...
		$Query  = "select * from state_master WHERE state_name = '".$state_name."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from state_master WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	if ($rsu[0]['status']== 1)
	{
	$Query  = "UPDATE state_master SET status = 0 WHERE id = ".$row_id."";
	}
	elseif ($rsu[0]['status']== 0)
	{
	$Query  = "UPDATE state_master SET status = 1 WHERE id = ".$row_id."";
	}
	#echo $Query;
	#die();
	$objDB->setQuery($Query);
	$rs = $objDB->update();
	$_SESSION[SUCCESS_MSG] = "State Status Changed successfully...";
	header("location: index.php?p=".$_REQUEST['p']."");
	exit();
	}
}
