...
$objDB = new DB();
if(isset($_GET['status']) && $_GET['status'] == 'del' && $_GET['did'] != "") {
    
    $id = base64_decode($_GET['did']);  
    
    $update = "update pis_master_ge_short_premium_renewal set is_deleted = '1' where id = ".$id;    
    $objDB->setQuery($update);
    $objDB->update();        
    
    $select = "select * from pis_master_ge_short_premium_renewal where id = ".$id;    
    $objDB->setQuery($select);
    $rs = $objDB->select();     
    
    $pis_mode = $rs[0]['pis_mode'];  
    //echo $pis_mode;
    ...
    $update1 = "update installment_master_ge_short_premium_renewal   set  ".$updcol."  = '0' where  $updcol = ".$id;     
    $objDB->setQuery($update1);
    $objDB->update(); 
    
    header("Location:".URL."webadmin/index.php?p=pis_branch_short_renewal_ge");   
...
if(isset($_SESSION['branch_name_id']) && $_SESSION['branch_name_id'] != '') 
	...
	##### CODE FOR SEARCHING 
$Query = "select count(id) as CNT from pis_master_ge_short_premium_renewal ".$where;
#echo $Query;
...
$pageRecordCount = 0;
$Query = "SELECT * FROM pis_master_ge_short_premium_renewal ".$where.$OrderBY.$Limit;
#echo $Query;
#print_r($_POST);
  ...
  <tbody>
    <tr> 
      ...
      <td colspan="20" align="right">
        <a title=" Export to Excel " href="<?=URL?>pis_excel_short_renewal_ge.php" ><img src="images/excel_icon.gif" border="0" title="Download" /></a>
      </td>
    </tr>
		<?php
			}	
	...
	<tr> 
      <td colspan="20" align="left">
				<table>
				<tr>
						<td colspan="3" align="center"><strong>Branch PIS<br/><br/></strong></td>
					</tr>
					<tr> 
					...
					<?php
						...
						{
					?>
					<?php if($_SESSION[ROLE_ID]=="1" || $_SESSION[ROLE_ID]=="2" || $_SESSION[ROLE_ID]=="7"): ?>
					<tr>
						...
						<?php 
								...
								$branch_sql = 'select * from admin where  role_id = 4 ORDER BY branch_name ASC';
								//echo $branch_sql;
					...
					<tr>
					  <td colspan="3" align="center"><input type="submit" name="btnSubmit" value="Search" class="inplogin">
						...
						if($_SESSION[ROLE_ID] != '7'){  //============ Edited  =============
						?>
						<input type="button" name="btnPis" value="Generate PIS" class="inplogin" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_pis_short_renewal_ge.php?pisfromdate=<?php echo $_POST['from_date']?>&pistodate=<?php echo $_POST['to_date']?>&pismode=<?php echo $_POST['pis_mode']?>')" />
						<?php
							}   //============ Edited  =============
						}	
						?>						</td>
...
?>
	...
	<tr> 			
			...
			<?php if($_SESSION[ROLE_ID] == '1' || $_SESSION[ROLE_ID] == '4'){ ?>			
			<td width="8%" align="center" valign="top" ><a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_pis_admin_short_renewal_ge.php?id=<?php echo base64_encode($mysql_id); ?>')">View</a></td>
            <?php }
			if($_SESSION[ROLE_ID] == '1'){ ?>
			<td width="8%" align="center" valign="top" >			
			<a href="<?php echo URL; ?>webadmin/index.php?p=pis_branch_short_renewal_ge&status=del&did=<?php echo base64_encode($mysql_id); ?>" onclick="return confirm('Do you want to delete this item?');">Cancel</a>
			</td>
			<?php } ?>
