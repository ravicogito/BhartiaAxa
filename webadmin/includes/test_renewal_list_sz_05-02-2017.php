...
$pid 			= loadVariable('pid',0);
	...
	if(isset($branch_name))
	{
	 ...
	 /* ------------------------------------------------------------------------------------------------------   
						   Modified By  : Ajit Kumar Gautam
						   Modified Date: 01/01/2016 
						   Purpose      : Adding Campaign Feild
...
else if($_SESSION[ROLE_ID] == '4')
{
 ...
 /* ------------------------------------------------------------------------------------------------------   
	...
	********************************************************************************************************/
	...
	if(isset($_SESSION['campaign_id']) && $_SESSION['campaign_id'] != '') // this is actually campaign id
	{
...
if(isset($_SESSION['hub_name_id']) && $_SESSION['hub_name_id'] != '') 
	...
	##### CODE FOR SEARCHING 
$Query = "select count(id) as CNT from installment_master_sz_renewal  ".$where;
$Query;
...
$Query = "SELECT * FROM installment_master_sz_renewal ".$where.$OrderBY.$Limit;
//echo $Query;
#exit;
$objDB->setQuery($Query);
$rs = $objDB->select();
  ...
  <tbody>
    <tr> 
      ...
      <td colspan="26" align="right">
        <a title=" Export to Excel " href="<?=URL?>test_transaction_excel_renewal_sz.php" ><img src="images/excel_icon.gif" border="0" title="Download" /></a>
      </td>
    </tr>
	<tr> 
      <td colspan="26" align="left">
            ...
            ********************************************************************************************************-->
                      ...
                      <td valign="top" align="left"><select name="campaign_id" id="campaign_id" class="inplogin"  style="width:150px;">
                        ...
                        <?php 
										...
										if($branch_id!=2 && $hub_id!=2 && $branch_id!=384)
										{
											$query.=" AND (branch_id='$branch_id')";
										}
										/*elseif($branch_id!=2 && $branch_id!=384)
										{
											if($hub_id==2)
											{
    ...
    <tr class="TDHEAD"> 
      <td colspan="26">Transaction Listing</td>
    </tr>
    <!--
	<tr>
	...
	if($pageRecordCount > 0)
	{
		for($i=0;$i<count($rs);$i++) { 
		 ...
		 else
		 {
			...
			//$data_campaign=mysql_fetch_assoc($rsCampaign);
...
?>
	<tr> 
            ...
            ********************************************************************************************************-->
			...
			<td width="8%" align="center" valign="top" ><?php echo stripslashes($rs[$i]['receive_cash']); ?></td>
			<td width="8%" align="center" valign="top" ><?php echo stripslashes($rs[$i]['receive_cheque']); ?></td>
			<td width="8%" align="center" valign="top" ><?php echo stripslashes($rs[$i]['receive_draft']); ?></td>
			
			<td width="8%" align="center" valign="top" ><?php echo stripslashes($rs[$i]['premium']); ?></td>
			
			<td width="8%" align="center" valign="top" ><?php echo stripslashes($rs[$i]['health']); ?></td>
			...
			<?php if($_SESSION[ROLE_ID] == '1' || $_SESSION[ROLE_ID] == '4'){ 
                        ...
                        <td width="8%" align="center" valign="top" > 
                            ...
                            <a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/test_window_edit_renewal_sz.php?id=<?php echo base64_encode($mysql_id); ?>')">Edit</a>
			...
			<td width="8%" align="center" valign="top" >
                        <?php if($rs[$i]['cash_pis_id'] == '0' && $rs[$i]['cheque_pis_id'] == '0' && $rs[$i]['draft_pis_id'] == '0'){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/renewal_sz_cancel.php?id=<?php echo base64_encode($mysql_id); ?>')">Cancel</a>
                        <?php } ?>
			</td>
			<?php } ?>
