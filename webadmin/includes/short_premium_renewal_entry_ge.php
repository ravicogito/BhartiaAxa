...
if($msg=="")
{
			...
			foreach($xml->GTFScolldata as $v)
			{
				...
				if($company_code!='2000000355')
				{
					...
					header("location: ".URL.'webadmin/index.php?p=short_premium_renewal_entry_ge');
					exit;
				}
				...
				// $tot_receipt=mysql_num_rows(mysql_query("select id from  installment_master_ge_short_premium_renewal where $database_field='$money_receipt' and is_deleted='0'"));
				
				$tot_receipt=mysql_num_rows(mysql_query("select id from installment_master_ge_short_premium_renewal where (cash_money_receipt='$money_receipt' || cheque_money_receipt='$money_receipt' || draft_money_receipt='$money_receipt') and is_deleted='0'"));
				...
				if($tot_receipt==0 && strtolower($type_of_business)!='rwp' && strtolower($type_of_business)!='fpr' && $policy_no!="" && strtolower($reciept_status)=='live')
				{
					...
					//-------- start for check application number ---------//
						$query_policy_no=mysql_query("select id from installment_master_ge_short_premium_renewal where policy_no='$policy_no' and is_deleted='0'");
					...
					// if($tot_policy_no==0) 
					{
						...
						*/
						...
						mysql_query("insert into installment_master_ge_short_premium_renewal set $query");
					...
					/*else
					{
						...
						mysql_query("update installment_master_ge_short_premium_renewal set $query where id='$id'");
			...
			header("location: ".URL.'webadmin/index.php?p=short_premium_renewal_list_ge');
}
...
<TABLE class="table_border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <tbody>
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Manual Entry (Short Premium - Renewal - GE)</td>
    </tr>
