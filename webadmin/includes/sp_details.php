...
function show_list(){
	...
	$sortField 		= loadVariable('sf','sp_name');
...
$where = " WHERE 1 ";
	if($searchField=="sp_name"){
		$where .= " AND sp_name = '".mysql_real_escape_string($searchString)."' ";
	}
        if($searchField=="sp_code"){
		$where .= " AND sp_code = '".mysql_real_escape_string($searchString)."' ";
	}
//echo $where;
$OrderBY = " ";
if($sortField <> "" && $sortType <> "")
{
	$OrderBY .= " ORDER BY sp_name ".$sortType;
}
//=======================================================
$Query = "select count(id) as CNT from sp_master  ".$where;
...
$Query = "select  *  from sp_master ".$where.$OrderBY.$Limit;
#echo $Query;
$objDB->setQuery($Query);
$rs = $objDB->select();
...
<form name="searchFrm" id="searchFrm" action="" method="post" onsubmit="return check_search();">
  <TABLE class=border cellSpacing=1 cellPadding=5 width="100%" align=center border=0 >
    ...
    <TR class=TDHEAD> 
      <TD colSpan=3>Insurance SP Name Management Panel</TD>
    </TR>
    <TR> 
		...
		<select name="searchField" id="searchField" class="inplogin"  >
			...
			<option value="sp_name" <?php if($searchField=='sp_name') echo 'selected';?>>SP Name </option>
                        <option value="sp_code" <?php if($searchField=='sp_code') echo 'selected';?>>SP Code </option></td>
      <TD width="85%">
	  
	  <INPUT class="inplogin" name="search" id="search" value="<?=stripslashes($searchString)?>"> 
        <INPUT class="inplogin" type="submit" value="Search"> <INPUT class="inplogin" onclick="window.location.href='index.php?p=sp_master&mid=<?=$mid?>&all=1'" type="button" value="Show All" name="btnShowAll">
      </TD>
    </TR>
  </TBODY>
</TABLE>
...
<TABLE cellSpacing=1 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
      ...
      <TD width="93%" align=left valign="middle" ><?php include_once("../utility/pagination_display.php");?></TD>
          <TD width="5%" align=right><A title=" Refresh the page" href="#" onClick="javascript:window.location.href='index.php?p=sp_master&mid=<?=$mid?>&pid=<?=$pid?>&all=1';"><IMG src="images/icon_reload.gif" border=0></A></TD>
...
<TABLE class="border" cellSpacing=2 cellPadding=5 width="100%" align=center border=0>
  <TBODY>
    ...
    <TR class="TDHEAD"> 
	  ...
	  <TD  width="15%" class="heading_text" align="left"> SP Name </TD>
    ...
    <TR class=body onmouseover="this.bgColor='#F7F7F7'" onmouseout="this.bgColor=''"> 
      ...
      <TD class="<?=$cls?>" align="left"><?=$rs[$i]['sp_name']." (".$rs[$i]['sp_code'].")"?></TD>
...
{ 
	...
	{	
		$current_mode = "Edit";
		$Query = "SELECT * FROM sp_master WHERE id=".$user_id;		
		/*$rs  = mysql_query($sql);
	 ...
	 <tr>
	  	<td >
			<table cellSpacing=2 cellPadding=5 width="99%" align=center border="0">
			  ...
			  <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">SP Name <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="sp_name" id="sp_name" size="40" <?php if(isset($rec[0]['sp_name']) && $rec[0]['sp_name']!="") {?> value="<?=stripslashes($rec[0]['sp_name'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
                          
                          <tr>
				<td width="23%" align="right" valign="top" class="tbllogin">SP Code <font color="#ff0000">*</font></td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
					<input type="text" class="validate[required] inplogin" name="sp_code" id="sp_code" size="40" <?php if(isset($rec[0]['sp_code']) && $rec[0]['sp_code']!="") {?> value="<?=stripslashes($rec[0]['sp_code'])?>" <? }?> onKeyUp="this.value = this.value.toUpperCase();" />
				</td>
			  </tr>
                          
			<tr>
				<td width="23%" align="right" valign="top" class="tbllogin">Is Latest SP Name </td>
				<td width="2%" align="center" valign="top" class="tbllogin">:</td>
				<td width="75%" align="left" valign="top">
...
{
	$objDB = new DB();
	$sp_name 					= loadVariable('sp_name',''); //sp_code
        $sp_code 					= loadVariable('sp_code','');
	...
	if ($row_id=='')
	{
		...
		$Query  = "select * from sp_master WHERE sp_name = '".$sp_name."' ";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
			$_SESSION[ERROR_MSG] = "SP Name already exists...";
			header("location: index.php?p=".$_REQUEST['p']."");
			exit();
		}
		...
		$Query  = " INSERT INTO sp_master SET "; //$sp_code
		$Query .= " sp_name 							= '".$sp_name."',";
                $Query .= " sp_code 							= '".$sp_code."',";
		...
		//================================
		$_SESSION[SUCCESS_MSG] = "SP Name Added successfully...";
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
	else
	{
		...
		$Query  = "select * from sp_master WHERE sp_name = '".$sp_name."'  and id<>'".$row_id."'";
		$objDB->setQuery($Query);
		$rsu = $objDB->select();
		if(count($rsu) > 0 )
		{
		...
		$_SESSION[SUCCESS_MSG] = "SP Name updated successfully...";
		$objDB->close();
		header("location: index.php?p=".$_REQUEST['p']."");
		exit();
	}
...
function user_status($row_id = '')
{
	...
	if($row_id)
	{
	$Query  = "select * from sp_master WHERE id = ".$row_id."";
	$objDB->setQuery($Query);
	$rsu = $objDB->select();
	if ($rsu[0]['status']== 1)
	{
	$Query  = "UPDATE sp_master SET status = 0 WHERE id = ".$row_id."";
	}
	elseif ($rsu[0]['status']== 0)
	{
	$Query  = "UPDATE sp_master SET status = 1 WHERE id = ".$row_id."";
	}
	#echo $Query;
	#die();
	$objDB->setQuery($Query);
	$rs = $objDB->update();
	$_SESSION[SUCCESS_MSG] = "SP Name Status Changed successfully...";
	header("location: index.php?p=".$_REQUEST['p']."");
	exit();
	}
}
