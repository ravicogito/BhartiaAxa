...
$Query = "select count(id) as CNT from renewal_short_premium_gtfs  ".$where;
//echo $Query;
//exit;
...
$Query = "SELECT * FROM renewal_short_premium_gtfs ".$where.$OrderBY.$Limit;
  ...
  <tbody>
    <tr> 
      ...
      <td colspan="25" align="right">
        <a title=" Export to Excel " href="<?=URL?>short_premium_renewal_excel_admin_gtfs.php" ><img src="images/excel_icon.gif" border="0" title="Download" /></a>
      </td>
    </tr>
...
?>
	<tr> 
	  ...
	  <td width="8%" align="center" valign="top" ><?php echo (intval($rs[$i]['is_duplicate']) != 0 ? 'Yes' : 'No'); ?>
	  
	   <br />
			<?php if($_SESSION[ROLE_ID] == '1'){if($rs[$i]['is_duplicate'] != ""){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_r_gtfs_duplicate_edit.php?id=<?php echo base64_encode($rs[$i]['id']); ?>')">Edit<?php }} ?></td>
			...
			<?php if($_SESSION[ROLE_ID] == '1'){if($rs[$i]['receipt_number'] != ""){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_r_gtfs_receipt_no_edit.php?id=<?php echo base64_encode($rs[$i]['id']); ?>')">Edit<?php }} ?>			
			...
			<br />
			<?php if($_SESSION[ROLE_ID] == '1'){if($branch_name != ""){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_r_gtfs_branch_edit.php?id=<?php echo base64_encode($rs[$i]['id']); ?>')">Edit<?php }} ?>			
			</td>
			<td width="8%" align="center" valign="top" ><?php echo $plan; ?><br />
			<?php if($_SESSION[ROLE_ID] == '1'){if($plan != ""){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_r_gtfs_plan_edit.php?id=<?php echo base64_encode($rs[$i]['id']); ?>')">Edit<?php }} ?>
			</td>
			<td width="8%" align="center" valign="top" ><?php echo date('d-m-Y', strtotime($rs[$i]['deposit_date'])); ?></td>
			<?php $due_date = '';
			if(($rs[$i]['due_date'] != '0000-00-00' ) && ($rs[$i]['due_date'] != '1970-01-01' )){
			$due_date = date('d-m-Y',strtotime($rs[$i]['due_date']));
			}?>
			<td width="8%" align="center" valign="top" ><?php echo $due_date; ?>
			<br />
			<?php if($_SESSION[ROLE_ID] == '1'){if($due_date != ""){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_r_gtfs_due_date_edit.php?id=<?php echo base64_encode($rs[$i]['id']); ?>')">Edit<?php }} ?>
			...
			<td width="8%" align="center" valign="top" ><?php echo $rs[$i]['amount']; ?><br />
			<?php if($_SESSION[ROLE_ID] == '1'){if($rs[$i]['amount'] != ""){ ?>
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_r_gtfs_amount_edit.php?id=<?php echo base64_encode($rs[$i]['id']); ?>')">Edit<?php }} ?></td>
			...
			//} ?></td>	 -->
			<td width="8%" align="center" valign="top" >			
				<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_sp_renewal_gtfs.php?id=<?php echo base64_encode($mysql_id); ?>')">Generate</a>		
			</td>
			<?php
			...
			<a href="javascript:void(0)" onclick="javascript:popUp('<?php echo URL; ?>webadmin/window_cancel_renewal_short_gtfs.php?id=<?php echo base64_encode($mysql_id); ?>')">Cancel</a>
			<?php } ?>
