...
function find_application_id($application_no) // This function will determine whether this is coming from GAP
	{
		$objDB = new DB();
		$gap_status = 0;
		$selGap_status = mysql_query("SELECT id FROM sicl_app_master WHERE 	application_no='".$application_no."'");
...
print_r($_FILES['zip']);
	...
	if(isset($_FILES['zip']) && $_FILES['zip']['tmp_name'] != '')
	{
		...
		//$filename = 'sicl_policies/test_policies.xls';
		...
		for ($x = 2; $x <= count($data->sheets[0]["cells"]); $x++) {
			...
			$firstInsert = "INSERT INTO sicl_app_master SET
			...
			{
				$firstUpdate = "update sicl_app_master SET 
    ...
    <tr> 
      <td colspan="3" style="padding-left: 70px;" align="left"><b><font color="#ff0000">All 
        * marked fields are mandatory</font></b></td>
		...
		<tr> 
      <td class="tbllogin" valign="top" align="right">Upload SICL Policy Number (In .xls format)<font color="#ff0000">*</font></td>
