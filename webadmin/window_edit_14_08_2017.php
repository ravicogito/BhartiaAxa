...
if(isset($_GET['id']) && !empty($_GET['id']))
{
	...
	if(isset($_POST['submit']) && $_POST['submit'] != '')
	{
		...
		// Secondary Entry variables
				...
				if(!isset($phase)) { $phase = ''; } else  { $agent_code = addslashes($agent_code); }
	...
	if($numTransaction > 0)
		{
            ...
            $address = stripslashes($getTransaction['insured_address1']);
          ...
          /*$agent_code = stripslashes($getTransaction['agent_code']);
			...
			$identity_proof = stripslashes($getTransaction['identity_proof']);
			$address_proof = stripslashes($getTransaction['address_proof']);
...
<?php if(isset($_SESSION[ROLE_ID]) && ($_SESSION[ROLE_ID] == '1')) // Superadmin Preliminary Edit
	...
	<tr> 
      ...
      <td class="tbllogin" valign="top" align="center">:</td>
      <td valign="top" align="left"><input name="applicant_dob" id="applicant_dob" type="text" class="inplogin"  value="<?php if($applicant_dob!="") { echo date("d/m/Y",strtotime($applicant_dob)); } ?>" maxlength="20" onchange="ageCount('applicant_dob');" readonly /> <!-- <font color="#ff0000">(DD-MM-YYYY)</font> -->&nbsp;<img src="images/cal.gif" alt="" style="border: 0pt none ; cursor: pointer; position: absolute;" onclick="displayCalendar(document.addForm.applicant_dob,'dd/mm/yyyy',this)" width="20" height="18"><br><a onClick="clear_input('applicant_dob');" style="cursor:pointer">Clear</a></td>
    </tr>
	...
	<tr> 
	  ...
	  <td class="tbllogin" valign="top" align="center">&nbsp;</td>
	  <td align="left" valign="top"><input name="copy_name" id="copy_name" type="checkbox" class="inplogin" onclick="copy_details();">Copy name from primary</td>
    </tr>	
        ...
        <tr> 
                      ...
                      <!--	  <input type="hidden" name="business_type" id="business_type" value="2" >-->
                                ...
                                <select name="business_type" id="business_type" class="inplogin">
                                                      ...
                                                      ?>
                                                              <option value="<?php echo $getPlan['id']; ?>" <?php echo ($getPlan['id'] == $type_of_business ? 'selected' : ''); ?>><?php echo $getPlan['business_type']; ?></option>
                                                      <?php
                                                                      }
                                                              }
                                                      ?>
