...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_installment = "UPDATE temporary_installment_master SET tenure = '".$charge."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT tenure FROM temporary_installment_master WHERE id='".$app_id."'";
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Update Term</td>
    </tr>
		
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Term</td>
	 ...
	 <select name="charge" id="" class="inplogin_select" style="width:140px;">
							...
							<?php 
								$selBranch = mysql_query("SELECT tenure, id FROM tenure_master WHERE status=1 ORDER BY tenure ASC");
								while($getBranch = mysql_fetch_array($selBranch))
								{					
							?>
								<option value="<?php echo $getBranch['tenure'];?>" <?php echo (isset( $value) && ($value  == $getBranch['tenure']) ? 'selected' : ''); ?>><?php echo $getBranch['tenure'];?></option>
							<?php } ?>
							</select>
	  </td>
    </tr>
