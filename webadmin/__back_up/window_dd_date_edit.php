...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		//echo $charge;
		//echo "<br/>";
		//echo date('Y-m-d',strtotime($charge));
		//exit;
		//$update_folio = "UPDATE  customer_folio_no SET application_no = '".$charge."' WHERE id = '".$app_id."'";
		$update_installment = "UPDATE installment_master SET dd_date = '".date('Y-m-d',strtotime($charge))."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT dd_date FROM installment_master WHERE id='".$app_id."'";
...
<html>
 <head>
  <title> Edit Cheque/DD Date </title>
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Update Cheque/DD Date</td>
    </tr>
		
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Cheque/DD Date</td>
      <td width="5%" align="center" valign="top" class="tbllogin">:</td>


	  <td valign="top" align="left"><input name="charge" id="charge" type="text" class="inplogin"  value="<?php echo str_replace('/', '-', $value ); ?>" maxlength="20" readonly /> <!-- <font color="#ff0000">(DD-MM-YYYY)</font> -->&nbsp;<img src="images/cal.gif" alt="" style="border: 0pt none ; cursor: pointer; position: absolute;" onclick="displayCalendar(document.addForm.charge,'dd-mm-yyyy',this)" width="20" height="18"></td>
