...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_installment = "UPDATE renewal_short_premium_gtfs SET due_date = '".date('Y-m-d',strtotime($charge))."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT due_date FROM renewal_short_premium_gtfs WHERE id='".$app_id."'";
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Update Due Date</td>
    </tr>
		
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Due Date</td>
