...
{
	extract($_POST);
	mysql_query("UPDATE pis_master_sicl_renewal SET cms_no='".$cms_no."', deposit_date='".date('Y-m-d', strtotime($deposit_date))."' WHERE id='".$previousID."'");
...
{
	$selOldRec = mysql_query("SELECT * FROM pis_master_sicl_renewal WHERE id='".$previousID."'");
...
$where = "WHERE is_deleted=0 AND (pis_id='".base64_decode($_GET['id'])."' OR alt_pis_id='".base64_decode($_GET['id'])."')";
$OrderBY = " ORDER BY id DESC ";



$Query = "SELECT * FROM renewal_master ".$where.$OrderBY;
	 ...
	 <table style="width:770px;">		 
		 <tr>
			<td colspan="3" align="center">
				<form method="post" action="" name="cms_frm" onsubmit="return pisCheck()">
					...
					<strong>SARADA INSURANCE CONSULTANCY LIMITED<br />
					...
					<br />
					CMS No. : <input type="text" name="cms_no" id="cms_no" value="<?php echo $cms_no;?>"
		...
		?>
					...
					<tr>
						...
						<?php 
							//echo $rs[$i]['amount']; 
							$cash_amount = 0.00;
							if($rs[$i]['payment_mode'] == 'CASH')
							{
								$cash_amount += $rs[$i]['main_amount'];
								//$idstr_main .= $rs[$i]['id'].',';
							}
		...
		?>
					...
					<tr>
						...
						<td width="100" style="padding-top:5px;"><?php echo $rs[$i]['dd_number']; ?></td>
						<td width="100" style="padding-top:5px;"><?php echo $rs[$i]['dd_bank_name']; ?></td>
						<td width="50" style="padding-top:5px;text-align:right;">
						<?php 
							//echo $rs[$i]['amount']; 
							$dd_amount = 0.00;
							if($rs[$i]['payment_mode'] == 'DD')
							{
								$dd_amount += $rs[$i]['main_amount'];
								//$idstr_main .= $rs[$i]['id'].',';
							}

							if($rs[$i]['other_payment_mode'] == 'DD')
							{
								$dd_amount += $rs[$i]['other_amount'];
								//$idstr_alt .= $rs[$i]['id'].',';
							}
		...
		?>
					...
					<tr>
						...
						<?php 
							//echo $rs[$i]['amount']; 
							$cheque_amount = 0.00;
							if($rs[$i]['payment_mode'] == 'CHEQUE')
							{
								$cheque_amount += $rs[$i]['main_amount'];
								//$idstr_main .= $rs[$i]['id'].',';
							}

							if($rs[$i]['other_payment_mode'] == 'CHEQUE')
							{
								$cheque_amount += $rs[$i]['other_amount'];
								//$idstr_alt .= $rs[$i]['id'].',';
							}
