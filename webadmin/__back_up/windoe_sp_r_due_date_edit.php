...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_installment = "UPDATE renewal_short_premium SET due_date = '".date('Y-m-d',strtotime($charge))."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT due_date FROM renewal_short_premium WHERE id='".$app_id."'";
