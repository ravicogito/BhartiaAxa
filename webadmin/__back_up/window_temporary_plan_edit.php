...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_installment = "UPDATE temporary_installment_master SET plan = '".$charge."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT plan FROM temporary_installment_master WHERE id='".$app_id."'";
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
	...
	<tr> 
	 ...
	 <select name="charge" id="" class="inplogin_select" style="width:140px;">
							...
							<?php 
								$selBranch = mysql_query("SELECT plan_name, id FROM insurance_plan WHERE status=1 ORDER BY plan_name ASC");
								while($getBranch = mysql_fetch_array($selBranch))
								{					
							?>
								<option value="<?php echo $getBranch['id'];?>" <?php echo (isset( $value) && ($value  == $getBranch['id']) ? 'selected' : ''); ?>><?php echo $getBranch['plan_name'];?></option>
							<?php } ?>
							</select>
	  </td>
    </tr>
