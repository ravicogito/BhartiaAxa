...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_installment = "UPDATE renewal_short_premium_gtfs SET is_duplicate = '".$charge."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT is_duplicate FROM renewal_short_premium_gtfs WHERE id='".$app_id."'";
	$objDB->setQuery($selTransaction);
	$rs = $objDB->select();

	//print_r($rs);
	$value = $rs[0]['is_duplicate'];
	//exit;
...
<html>
 <head>
  <title> Edit Duplicate Entry</title>
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Update Duplicate Entry</td>
    </tr>
		
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Is Duplicate</td>
	 ...
	 <select name="charge" id="" class="inplogin_select" style="width:140px;">
							...
							<?php 
								...
								//{					
							?>
								<option value=2 <?php echo (isset( $value) && ($value  == 0) ? 'selected' : ''); ?>>No</option>
								<option value=1 <?php echo (isset( $value) && ($value  == 1) ? 'selected' : ''); ?>>Yes</option>
