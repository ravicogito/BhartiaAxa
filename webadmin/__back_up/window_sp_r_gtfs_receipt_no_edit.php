...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_folio = "UPDATE renewal_short_premium_gtfs  SET receipt_number = '".$charge."' WHERE id = '".$app_id."'";		//echo $update_folio;
		//exit;
...
<?php
	...
	$selTransaction = "SELECT receipt_number FROM renewal_short_premium_gtfs WHERE id='".$app_id."'";
	$objDB->setQuery($selTransaction);
	$rs = $objDB->select();

	//print_r($rs);
	$value = $rs[0]['receipt_number'];
	//exit;
...
<html>
 <head>
  <title> Edit Receipt Serial Number </title>
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
    ...
    <tr class="TDHEAD"> 
      <td colspan="3">Update Receipt Serial  No.</td>
    </tr>
		
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Receipt Serial  No.</td>
      <td width="5%" align="center" valign="top" class="tbllogin">:</td>
      <td width="68%" align="left" valign="top"><input type="textbox" id="charge" name="charge" value="<?php echo $value; ?>" onkeyPress="return keyRestrict(event,'0123456789')" /></td>
    </tr>
