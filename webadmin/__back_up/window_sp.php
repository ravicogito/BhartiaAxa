...
function find_receipt_status($receipt_id)
{
	...
	//echo $invoice_id;
	$selTransaction = mysql_query("SELECT receipt_generated FROM insurance_short_premium WHERE id='".$receipt_id."'");
...
if(isset($_GET['id']) && !empty($_GET['id']))
{
	...
	//echo $invoice_id;
	$selTransaction = mysql_query("SELECT * FROM insurance_short_premium WHERE id='".$invoice_id."'");
	$numTransaction = mysql_num_rows($selTransaction);
	if($numTransaction > 0)
		{
			$getTransaction = mysql_fetch_assoc($selTransaction);
	...
	<br/><br/>
	 ...
	 if(isset($getTransaction['applicant_name'])) {
		...
		<?php if(($getTransaction['sp_payment_mode'] == 'CHEQUE') || ($getTransaction['sp_payment_mode'] == 'DD')){?>Cheque/Demand Draft No <?php echo $getTransaction['sp_dd_no']; ?> dated <?php echo date('d-m-Y',strtotime($getTransaction['sp_dd_date'])); ?> drown on <?php echo $getTransaction['sp_dd_bank']; ?><?php }?>
			towards FP(<?php if($getTransaction['is_duplicate']=='1'){echo "Duplicate Entry Premium";}else{echo "Short Premium";}?>) in relation to above Proposal for  & on behalf of Life Insurance Corporation of India under<strong> <?php echo $data['plan_name'];?> </strong> and term<strong> <?php echo $getTransaction['term']; ?></strong> Years.</div>
	<br/>
<br/><br/>
...
<?php
	mysql_query("UPDATE insurance_short_premium SET receipt_generated=1 WHERE id='".base64_decode($_GET['id'])."'");
