...
$app_id = base64_decode($_GET['id']); 
	...
	if(isset($_POST['charge']) && !empty($_POST['charge']))
	{
		...
		$update_installment = "UPDATE temporary_installment_master SET application_no = '".$charge."' WHERE id = '".$app_id."'";
...
<?php
	...
	$selTransaction = "SELECT application_no FROM temporary_installment_master WHERE id='".$app_id."'";
	$objDB->setQuery($selTransaction);
	$rs = $objDB->select();

	//print_r($rs);
	$value = $rs[0]['application_no'];
	//exit;
...
<html>
 <head>
  <title> Edit Application Number </title>
...
<!--
	...
	{	
		...
		{
			alert("Please Enter Apllication No.");
...
<script type="text/javascript" src="<?=URL?>js/site_scripts.js"></script>
 ...
 <form name="addForm" id="addForm" action="" method="post" onsubmit="return dochk()">
	...
	<tr> 
      <td width="27%" align="right" valign="top" class="tbllogin">Application No.</td>
