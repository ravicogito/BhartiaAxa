...
$sql="SELECT * FROM installment_master_sz_renewal WHERE is_deleted=1 ".$where." ORDER BY id DESC";
// echo $sql;
// exit;
...
$numTransaction = mysql_num_rows($selTransaction);
			...
			while($getTransaction = mysql_fetch_assoc($selTransaction))
			{
				...
				else
				{
					$line .= "Yes"."\t";
                                ...
                                $line .= find_branch_code($getTransaction['branch_id'])."\t";
				...
				$line .= stripslashes($getTransaction['health'])."\t";
                                
                                $line .= $getTransaction['cancellation_reason']."\t";
	...
	header("Content-Disposition: attachment; filename=renewal_sz".'_'.date('m-d-Y_H:i').".xls");
