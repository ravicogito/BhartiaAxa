...
$sql="SELECT * FROM installment_master_sz_short_premium_fresh WHERE is_deleted=1 ".$where." ORDER BY id DESC";
 //echo $sql;
// exit;
	...
	//$rs = $objDB->select();
		...
		if($numTransaction > 0)
		{
			while($getTransaction = mysql_fetch_assoc($selTransaction))
			{
				...
				$line = '';
				$deposit_date = '';
				if(($getTransaction['deposit_date'] != '0000-00-00')&&($getTransaction['deposit_date'] != '1970-01-01')){
